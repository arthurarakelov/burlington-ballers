...
const GameDetails = ({ game, user, onBack, onJoinGame, onLeaveGame, onDeclineGame, onMaybeGame, onDeleteGame, onEditLocation, onEditTime, editTrigger, hideHeader }) => {
  ...
  // Lazy-load all users only when the section scrolls into view
  const [usersRequested, setUsersRequested] = useState(false);

  useEffect(() => {
    // Request users once RSVP data is available (small user base, one-time load)
  ...
  // Shared RSVP action section with time picker
  const renderTimePickerActions = (promptText, buttons) => (
    <div className="space-y-4">
      {promptText && <h4 className="text-center text-sm font-light text-gray-500">{promptText}</h4>}
      ...
      <div className="relative z-10">
        ...
        {hasntResponded ? (
          <div className="space-y-5">
            ...
            {renderTimePickerActions(null, (
              <div className="grid grid-cols-2 gap-4">
                <Button onClick={handleMaybeGame} variant="secondary" loading={loadingStates.maybe}>
                  Maybe
                </Button>
                <Button onClick={handleDeclineGame} variant="secondary" loading={loadingStates.declining}>
                  Can't make it
                </Button>
              </div>
            ))}
          ...
          <div className="space-y-5">
            ...
            {renderTimePickerActions('Ready to commit?', (
              <Button onClick={handleDeclineGame} variant="secondary" loading={loadingStates.declining} className="w-full">
                Can't make it
              </Button>
            ))}
          </div>
          ...
          <div className="space-y-5">
            ...
            {renderTimePickerActions('Change your mind?', (
              <Button onClick={handleMaybeGame} variant="secondary" loading={loadingStates.maybe} className="w-full">
                Maybe
              </Button>
            ))}
          </div>
